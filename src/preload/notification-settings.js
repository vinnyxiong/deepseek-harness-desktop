...
contextBridge.exposeInMainWorld('desktopNotifications', Object.freeze({
  ...
  onRefresh(callback) {
    ...
    ipcRenderer.on('notification:refresh', listener);
    return () => ipcRenderer.removeListener('notification:refresh', listener);
  },
  onLocaleChanged(callback) {
    const listener = (_event, value) => callback(value);
    ipcRenderer.on('notification:locale', listener);
    return () => ipcRenderer.removeListener('notification:locale', listener);
  },
}));
